...
public class Character5Command implements Command {
    int playerIndex;
    int islandIndex;

    public Character5Command(int islandIndex, int playerIndex) {
        this.playerIndex = playerIndex;
        this.islandIndex = islandIndex;
    }

    /**
     * This method activate Character5 effect
        ...
        for (Player p : Game.getInstance().getBoard().getPlayersInGame()) {
            if (p.getPlayerIndex() == playerIndex) {
                if(islandIndex < Game.getInstance().getBoard().getIslandsInGame().size()){
                    Game.getInstance().getBoard().getCharacter5().activateTheEffect(p, islandIndex);
                }
                else{
