...
public class Character11Command implements Command {
    int playerIndex;
    PawnColor pawnColor;

    public Character11Command(PawnColor pawnColor, int playerIndex) {
        this.pawnColor = pawnColor;
        this.playerIndex = playerIndex;
    }

    /**
     * This method activate Character11 effect
    ...
    public void execute() throws GameException, InterruptedException {
        for (Player p : Game.getInstance().getBoard().getPlayersInGame()) {
            if (p.getPlayerIndex() == playerIndex) {
                Game.getInstance().getBoard().getCharacter11().activateTheEffect(p, pawnColor);
            }
        }
    }
}
